...
async def get_user_api(user: dict):
    try:
        return get_user(user.get("email"),
                        user.get("password"))
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def create_user_api(user: dict):
    try:
        return create_user(user.get("name"),
                           user.get("email"),
                           user.get("password"))
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/add_search_to_history",
...
async def add_search_to_history_api(search: dict):
    try:
        return add_search_to_history(search.get("chat_id"),
                                     search.get("first_message"),
                                     search.get("user_email"))
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/update_search_in_history",
        summary="Mettre à jour une recherche dans l'historique",
        response_description="Recherche mise à jour avec succès")
async def update_search_in_history_api(chat_id: int):
    try:
...
async def add_message_to_chat_api(message: dict):
    try:
        return add_message_to_chat(message.get("chat_id"),
...
@app.post("/delete_search_from_history",
        summary="Supprimer une recherche de l'historique",
        response_description="Recherche supprimée avec succès")
async def delete_search_from_history_api(chat_id: int):
    try:
        return delete_search_from_history(chat_id)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
