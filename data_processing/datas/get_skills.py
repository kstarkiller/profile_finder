...
if os.name == 'posix':
    psarm_path = 'data_processing/datas/sources/UC_RS_LP_RES_SKILLS_DETLS_22_1440892995.xlsx'
...
formatted_descriptions = ', '.join(f'"{escape_apostrophes(desc)}"' for desc in descriptions if pd.notna(desc))
formatted_profil = ', '.join(f'"{escape_apostrophes(profil)}"' for profil in profil if pd.notna(profil))
