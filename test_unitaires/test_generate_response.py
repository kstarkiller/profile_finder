...
class TestGenerateOllamaResponse(unittest.TestCase):
    ...
    def test_generate_ollama_response_invalid_model(self):
        response = generate_ollama_response('test_data', 'test_question', 'invalid_model')
        self.assertEqual(response, "The model invalid_model is not available. Please choose a valid model from this list: ['nomic-embed-text:latest', 'all-minilm:33m', 'llama3.1:8b']")
...
class TestGeneratePerplexityResponse(unittest.TestCase):
    def setUp(self):
        ...
        patch('generate_response.requests.post', return_value=self.mock_response).start()
    ...
    def test_generate_perplexity_response_success(self):
        response = generate_perplexity_response('test_data', 'test_question', 'llama-3.1-70b-instruct')
        self.assertEqual(response, 'Test response')

    def test_generate_perplexity_response_error(self):
        # Simulate a bad response with status code 500
        self.mock_response.raise_for_status.side_effect = requests.exceptions.HTTPError("500 Server Error")
    ...
    def test_generate_perplexity_response_no_data(self):
        response = generate_perplexity_response('', 'test_question', 'llama-3.1-70b-instruct')
        self.assertEqual(response, "Sorry, no data was provided. I need data to respond to your question. Please provide the data and try again.")

    def test_generate_perplexity_response_no_question(self):
        response = generate_perplexity_response('test_data', '', 'llama-3.1-70b-instruct')
        self.assertEqual(response, "I don't have a question to respond to. Please provide a valid question.")

    def test_generate_perplexity_response_question_too_long(self):
        response = generate_perplexity_response('test_data', 'a' * 513, 'llama-3.1-70b-instruct')
        self.assertEqual(response, "The question is too long. Please provide a question with less than 512 characters.")
