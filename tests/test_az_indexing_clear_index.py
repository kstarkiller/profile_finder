...
class TestClearIndex(unittest.TestCase):
    ...
    def test_clear_index_success(self, mock_sleep):
        ...
        search_client.delete_documents.return_value = None  # Simuler la suppression réussie
        ...
        search_client.delete_documents.assert_any_call(documents=[{"id": "1"}, {"id": "2"}])
    ...
    def test_clear_index_multiple_batches(self, mock_sleep):
        ...
        search_client.delete_documents.assert_any_call(documents=[{"id": "1"}, {"id": "2"}])
        search_client.delete_documents.assert_any_call(documents=[{"id": "3"}, {"id": "4"}])
    ...
    def test_clear_index_partial_failure(self, mock_sleep):
        ...
        with patch('builtins.print') as mocked_print:  # Patch print to check for error message
            clear_index(search_client, batch_size=2)
            mocked_print.assert_any_call("An error occurred while clearing the index: Random error")
